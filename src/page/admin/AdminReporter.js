...
const AdminStudent = ({ route, navigation }) => {
  ...
  const Edit_Select = (id) => {
    fetch(`http://52.221.184.135/API/Admin-System/ReporterManage/ReporterUpdate.php?id=${id}`, {
      method: 'GET',
    })
    .then((response) => response.json())
    .then((json) => {
      
      navigation.navigate("UpdateReporter", {
        id: json.id,
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: "ยืนยัน",
          onPress: () => {
            fetch(`http://52.221.184.135/API/Admin-System/ReporterManage/ReporterDelete.php?id=${id}`, {
              method: 'DELETE',
            })
              .then((response) => response.json())
              .then((json) => {
  ...
  <Text style={styles.headerText}>Reporter Account</Text>

  <TouchableOpacity 
  style={styles.addButton} 
  onPress={() => navigation.navigate('AddReporter')}
>
  <Text style={styles.addButtonText}>+ เพิ่มผู้รายงาน</Text>
</TouchableOpacity>
